...
# 5) 완만한 상승 채널 제거
# 6) "base 존재" 필터 추가
#    - 박스 이전에 의미 있는 하락/반등 구조가 있어야 함
#    - 상승 추세 중간의 횡보는 박스로 보지 않음
# ============================================================
...
RESULTS_FILE = os.path.join(OUTPUT_DIR, "rare_event_long_box_breakout.csv")
SUMMARY_FILE = os.path.join(OUTPUT_DIR, "rare_event_long_box_breakout_summary.json")
...
PRE_BASE_WINDOW = 120
PRE_BASE_DRAWDOWN_MIN = 0.18
BASE_POSITION_MAX = 0.40
BASE_RISE_MIN = 0.15
...
def state_rank(state: str) -> int:
    return {"돌파 임박": 0, "1차 돌파": 1, "후행 가능": 2}.get(state, 9)
...
def width_score(box_width: float) -> float:
    if 0.10 <= box_width <= 0.18:
        return 1.2
    if 0.08 <= box_width <= 0.22:
        return 1.0
    if 0.22 < box_width <= MAX_BOX_WIDTH:
        return 0.6
    return 0.0
...
def base_structure_filters_pass(
    ...
    pre_start = max(0, start_idx - PRE_BASE_WINDOW)
    pre_seg = full_df.iloc[pre_start:start_idx].copy()

    if len(pre_seg) < 40:
        return False

    pre_high = float(pre_seg["High"].max())
    pre_low = float(pre_seg["Low"].min())

    if pre_high <= 0 or pre_low <= 0 or pre_low >= pre_high:
        return False

    pre_drawdown = (pre_high - pre_low) / pre_high
    if pre_drawdown < PRE_BASE_DRAWDOWN_MIN:
        return False

    range_start = max(0, start_idx - 252)
    range_seg = full_df.iloc[range_start:start_idx + box_len].copy()
    ...
    base_position = (box_low - range_low) / (range_high - range_low)
    if base_position > BASE_POSITION_MAX:
        return False

    base_rise = (pivot_price - box_low) / box_low
    if base_rise < BASE_RISE_MIN:
        return False
...
def find_selected_box(df: pd.DataFrame) -> Optional[Dict[str, Any]]:
    ...
    for lookback in BOX_LOOKBACKS:
        ...
        if not base_structure_filters_pass(
            full_df=df,
            start_idx=start_idx,
            box_len=len(seg),
...
def save_outputs(results: List[LongBoxResult]) -> None:
    ...
    if not results:
        pd.DataFrame(columns=cols).to_csv(RESULTS_FILE, index=False, encoding="utf-8-sig")
