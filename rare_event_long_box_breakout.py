...
# ============================================================
# Long Box Breakout Scanner - RS Hard Cut Version
# ------------------------------------------------------------
# 핵심 변경
...
def calculate_total_score(box: Dict[str, Any], state: str, vol_ratio: Optional[float], rs_percentile: Optional[float], rs_current_vs_high: Optional[float], rs_new_high: bool) -> float:
    total = (
        box["score_length"] * 15
        + box["score_width"] * 15
        + box["score_touch"] * 14
...
def notify_changes(results: List[LongBoxResult]) -> None:
    ...
    for r in results:
        ...
        if not (
            ...
            and str(old.get("as_of_date")) == r.as_of_date
            and str(old.get("grade")) == r.grade
            and str(old.get("rs_grade")) == r.rs_grade
        ):
            new_alerts.append(r)
